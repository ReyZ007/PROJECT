...
describe("TaskController", () => {
  ...
  describe("Category Management", () => {
    ...
    test("should fail updating category for non-owned task", () => {
      // Arrange
      const otherUserData = TestDataFactory.createValidUserData({
        username: `testuser2-${Date.now()}`,
        email: `test2-${Date.now()}@example.com`,
      });
      const otherUser = userRepository.create(otherUserData);
      const otherUserController = new TaskController(taskRepository, userRepository);
      otherUserController.setCurrentUser(otherUser.id);
      const otherUserTask = otherUserController.createTask(TestDataFactory.createValidTaskData({ category: "work" })).data;

      // Switch back to test user
      taskController.setCurrentUser(testUser.id);

      // Act
      const response = taskController.updateTaskCategory(otherUserTask.id, "personal");

      // Assert
      TestAssertions.assertControllerResponse(response, false);
      expect(response.error).toBe("Hanya owner yang bisa mengubah kategori task");
    });
  });
});
